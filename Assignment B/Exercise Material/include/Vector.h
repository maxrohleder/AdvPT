// this file is just for documenting the required interface to compile with VectorTest.cpp (declarations only)
#include <iostream>
#include <assert.h>
...
public:
	// normal constructor
	Vector(size_t rows, size_t cols, T initValue) : no_cols(cols), no_rows(rows), data(new T[cols*rows]) {
		for (size_t i = 0; i < no_rows*no_cols; i++) data[i] = initValue;
	};
	// destructor
	~Vector() { delete[] data; };
	// copy constructor
	Vector(const Vector& n) : no_cols(n.no_cols), no_rows(n.no_rows), data(new T[n.no_cols*n.no_rows]) {
	...
	bool operator !=(const Vector& n) const { return !(*this == n); };
	...
	Vector operator +(const Vector& n) const {
		assert(no_rows == n.no_rows && no_cols == n.no_cols);
		Vector ret = *this;
		ret += n;
		return ret;
	...
	// Vector multiplication (pythonic matmul-symbol @ not overloadable)
	/*
	...
	*/
	Vector operator *(const Vector& n) const {
		...
		Vector<T> ret = Vector<T>(no_rows, n.no_cols, (T)0);
		size_t s1 = no_rows;
		size_t s2 = no_cols;
		size_t s3 = n.no_cols;
...
istream& operator >>(istream& in, const Vector& n) {
	for (size_t i = 0; i < n.no_cols*n.no_rows; i++)
	{
		in >> n.data[i];
	}
