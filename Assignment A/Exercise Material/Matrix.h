...
public:
	// normal constructor
	Matrix(size_t rows, size_t cols, double initValue) : no_cols(cols), no_rows(rows), data(new double[cols*rows]) {
		for (size_t i = 0; i < no_rows*no_cols; i++) data[i] = initValue;
	};
	// destructor
	~Matrix() { delete[] data; };
	// copy constructor
	Matrix(const Matrix& n) : no_cols(n.no_cols), no_rows(n.no_rows), data(new double[n.no_cols*n.no_rows]) {
	...
	Matrix& operator=(const Matrix& n) {
		...
		no_cols = n.no_cols;
		no_rows = n.no_rows;
		data = new double[no_cols*no_rows];
		for (size_t i = 0; i < no_rows*no_cols; i++) data[i] = n.data[i];
		return *this;
	};
	...
	*/
	Matrix operator *(const Matrix& n) const {
		...
		Matrix ret = Matrix(no_rows, n.no_cols, 0);
		size_t s1 = no_rows;
		size_t s2 = no_cols;
		size_t s3 = n.no_cols;
